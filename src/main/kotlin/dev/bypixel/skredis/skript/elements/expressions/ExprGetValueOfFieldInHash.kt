...
class ExprGetValueOfFieldInHash : SimpleExpression<String>() {
    fun register() {
        SkRedis.instance.addon.syntaxRegistry().register(
            ...
            DefaultSyntaxInfos.Expression.builder(ExprGetValueOfFieldInHash::class.java, String::class.java)
                .origin(Origin.of(SkRedis.instance.addon))
                .supplier { ExprGetValueOfFieldInHash() }
                .addPattern("value of field %string% in redis (hash|value) %string%")
                .build()
        )
    }
