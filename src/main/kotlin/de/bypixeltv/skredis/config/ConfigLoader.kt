...
object ConfigLoader {
    ...
    fun load() {
        ...
        configFile.parentFile?.let {
            if (!it.exists() && !it.mkdirs()) {
                Main.INSTANCE.sendErrorLogs("<red>Failed to create configuration directory!</red>")
            }
        } ?: return

        // If no config exists, create default
        if (!configFile.exists()) {
            Main.INSTANCE.sendLogs("<yellow>Configuration file not found! Creating a new one...</yellow>")
            config = Config()
        ...
        try {
            ...
            Main.INSTANCE.sendErrorLogs("<red>Failed to load configuration: ${e.message}</red>")
    ...
    fun save() {
        if (!configNeedsSaving && config != null) {
            Main.INSTANCE.sendLogs("<green>No changes to save</green>")
            return
        }
        ...
        try {
            ...
            Main.INSTANCE.sendLogs("<green>Configuration saved successfully</green>")
            configNeedsSaving = false
        } catch (e: Exception) {
            Main.INSTANCE.sendErrorLogs("<red>Failed to save configuration file: ${e.message}</red>")
        }
    }

    fun reload() {
        Main.INSTANCE.sendLogs("<yellow>Reloading configuration...</yellow>")
        config?.configVersion ?: -1
        load()
        Main.INSTANCE.sendLogs("<green>Configuration reloaded successfully!</green>")
    }
    ...
    private fun backupConfig(configFile: File) {
        try {
            ...
            if (!backupDir.exists() && !backupDir.mkdirs()) {
                Main.INSTANCE.sendErrorLogs("<red>Failed to create backup directory!</red>")
                return
            }
            val timestamp = System.currentTimeMillis()
            val backupFile = File(backupDir, "${configFile.nameWithoutExtension}-$timestamp.yml")
            Files.copy(configFile.toPath(), backupFile.toPath(), StandardCopyOption.REPLACE_EXISTING)
            Main.INSTANCE.sendLogs("<green>Created backup of config file at: ${backupFile.absolutePath}</green>")
        } catch (e: Exception) {
            Main.INSTANCE.sendErrorLogs("<red>Failed to create backup of config file: ${e.message}</red>")
        }
    }
    ...
    private fun mergeRecursive(defaultMap: Map<String, Any>, loadedMap: MutableMap<String, Any>, path: String): Boolean {
        ...
        if (keysToRemove.isNotEmpty()) {
            Main.INSTANCE.sendLogs("<blue>Found unknown keys at $path: $keysToRemove</blue>")
        }
        for (key in keysToRemove) {
            loadedMap.remove(key)
            changed = true
            Main.INSTANCE.sendLogs("<yellow>Removed unknown key: $path.$key</yellow>")
        }
        ...
        defaultMap.forEach { (key, defaultValue) ->
            if (!loadedMap.containsKey(key) || loadedMap[key] == null) {
                ...
                changed = true
                Main.INSTANCE.sendLogs("<yellow>Added missing key: $path.$key</yellow>")
